...
export class UI {
  ...
  action(action, data = {}) {
    ...
    if (action === 'play') cb.onPlay?.();
    ...
    else if (action === 'difficulty') { this.clearControls(); this.progress.difficulty = data.difficulty === 'extreme' ? 'extreme' : 'easy'; cb.onSettings?.({difficulty:this.progress.difficulty}); if(this.screen==='menu')this.showScreen('menu');else this.syncProgress(); }
    else if (action === 'leaderboard' || action === 'leaderboard-retry' || action === 'leaderboard-mode' || action === 'leaderboard-period') {
      ...
      if(action==='leaderboard')this.leaderboardPeriod='weekly';
      if(action==='leaderboard-period')this.leaderboardPeriod=data.period==='alltime'?'alltime':'weekly';
      this.leaderboard = {status:'loading',rows:[]};this.showScreen('leaderboard');cb.onLeaderboard?.(this.leaderboardDifficulty,this.leaderboardPeriod);
    }
  ...
  setFeaturedLeaderboard(groups) {this.featuredGroups=groups||{easy:[],extreme:[]};const preferred=this.featuredGroups[this.progress.difficulty]||[],fallback=this.featuredGroups[this.progress.difficulty==='easy'?'extreme':'easy']||[];this.featuredRanking=preferred[0]||fallback[0]||null;if(this.screen==='menu')this.showScreen('menu');}
  renderHomePodium() {
    const preferred=this.featuredGroups?.[this.progress.difficulty]||[],otherMode=this.progress.difficulty==='easy'?'extreme':'easy',fallback=this.featuredGroups?.[otherMode]||[];
    const rows=(preferred.length?preferred:fallback).slice(0,3),mode=preferred.length?this.progress.difficulty:otherMode,card=this.root.querySelector('.home-ranking-card');
    if(!rows.length||!card)return;
    const numeric=n=>Math.max(0,Math.floor(Number(n)||0)).toLocaleString('en-US');
    card.classList.add('home-podium-card');card.innerHTML=`<header><span class="home-rank-crown">♜</span><div><span class="eyebrow">WEEKLY PODIUM · ${esc(mode.toUpperCase())}</span><strong>Top 3 explorers</strong></div></header><ol>${rows.map((row,index)=>`<li><b>${index+1}</b><span>${githubLink(row.username,row.alias||row.username||'Explorer','Weekly ranked explorer')}<small>${numeric(row.score)} pts · ${numeric(row.levels)} levels</small></span></li>`).join('')}</ol><button data-action="leaderboard" data-mode="${esc(mode)}">View weekly rankings ↗</button>`;
  }
  renderLeaderboard() {
    const data=this.leaderboard, mode=this.leaderboardDifficulty,period=this.leaderboardPeriod;
    ...
    this.root.querySelector('#screen-content').innerHTML=`<section class="collection-page rankings-page"><div class="collection-heading"><div><span class="eyebrow">REAL RUNS. SHARED ADVENTURES.</span><h2>Make yourself <em>famous.</em></h2><p>Your campaign total is the sum of your verified personal best on every completed level.</p></div><button class="text-button" data-action="menu">← Back to menu</button></div><div class="rankings-toolbar"><div class="segmented" role="group" aria-label="Ranking difficulty"><button class="${mode==='easy'?'active':''}" aria-pressed="${mode==='easy'}" data-action="leaderboard-mode" data-mode="easy">Easy</button><button class="${mode==='extreme'?'active':''}" aria-pressed="${mode==='extreme'}" data-action="leaderboard-mode" data-mode="extreme">Extreme</button></div><span>Separate rankings. Same scoring logic.</span></div><section class="score-formula" aria-labelledby="score-formula-title"><div><span class="eyebrow">TRANSPARENT SCORING</span><h3 id="score-formula-title">Every point has a reason.</h3></div><div class="score-rule"><strong>1,000 + 50 × level</strong><span>Completion & progress</span></div><div class="score-rule"><strong>Up to 1,500</strong><span>Time efficiency</span></div><div class="score-rule"><strong>0 / 300 / 750</strong><span>1 / 2 / 3 lives left</span></div><div class="score-rule"><strong>100 · 250 · 500 · 50</strong><span>Coin · key · fruit · clock</span></div><div class="score-rule accent"><strong>${mode==='extreme'?'× 1.35':'× 1.00'}</strong><span>${mode==='extreme'?'Extreme physics':'Easy precision'}</span></div></section>${body}<aside class="rankings-explainer"><span>✦</span><p><strong>Your adventure, verified.</strong> The time bonus uses the percentage of available time remaining, so long levels do not receive free extra points. Required keys reward route complexity; optional collectibles reward exploration; surviving without mistakes matters. After completing a level, confirm the GitHub submission and the server recomputes every component automatically.</p></aside><p class="rankings-credit">ORBIT 2 by <a href="https://github.com/jaydemks" target="_blank" rel="noopener noreferrer">@jaydemks</a> · <a href="https://github.com/jaydemks/orbit_game_2" target="_blank" rel="noopener noreferrer">Explore the project ↗</a></p></section>`;
    this.root.querySelector('.score-rule strong').textContent='1,000 + 50 × (level − 1)';
    this.root.querySelector('.collection-heading p').textContent=period==='weekly'?'Your best verified scores from this Monday to Sunday. Everyone starts together each week.':'Your best verified score on every level across all seasons.';
    this.root.querySelector('.rankings-toolbar').insertAdjacentHTML('afterend',`<div class="ranking-periods" role="group" aria-label="Ranking period"><button class="${period==='weekly'?'active':''}" aria-pressed="${period==='weekly'}" data-action="leaderboard-period" data-period="weekly">This week</button><button class="${period==='alltime'?'active':''}" aria-pressed="${period==='alltime'}" data-action="leaderboard-period" data-period="alltime">All time</button><span>${period==='weekly'?'Resets Monday · 00:00 UTC':'Permanent personal bests'}</span></div>`);
    if(period==='weekly'&&data.hall?.length)this.root.querySelector('.rankings-explainer').insertAdjacentHTML('beforebegin',`<section class="hall-of-fame"><div><span class="eyebrow">HALL OF FAME</span><h3>Weekly champions</h3></div>${data.hall.map(row=>`<article><small>${esc(row.week)}</small><strong>${githubLink(row.username,row.alias||row.username,'Past weekly champion')}</strong><span>${numeric(row.score)} pts · ${esc(row.difficulty)}</span></article>`).join('')}</section>`);
  }
  ...
  syncSubmission() {
    ...
    this.root.querySelectorAll('.submission-status').forEach(el=>el.textContent=this.submission.message||(this.submission.available?'Confirm on GitHub. Validation then starts automatically.':'Complete a recorded run to submit a verified score.'));
  ...
  showScreen(name) {
    ...
    if(name==='menu'&&this.featuredGroups)this.renderHomePodium();
  ...
  showDialog(type) {
    ...
    if (type === 'won') {const b=this.snapshot.scoreBreakdown;body = `<p class="dialog-description">${campaignComplete ? 'You have explored all four worlds. Revisit them, beat your best scores and complete your collection.' : 'You found your way. A new perspective awaits on the other side.'}</p><div class="result-stats"><div><small>SCORE</small><strong>${this.snapshot.score || 0}</strong></div><div><small>COINS EARNED</small><strong>✦ ${this.snapshot.earned ?? this.snapshot.coins ?? 0}</strong></div></div>${b?`<div class="result-breakdown"><span>Completion <b>+${b.completion}</b></span><span>Exploration <b>+${b.collectibles}</b></span><span>Pace <b>+${b.pace}</b></span><span>Survival <b>+${b.survival}</b></span><strong>${b.multiplier>1?'Extreme × 1.35':'Easy × 1.00'}</strong></div>`:''}<button class="primary-button full" data-action="${campaignComplete ? 'worlds' : 'play'}">${campaignComplete ? 'Explore all worlds' : 'Next level'} <span>↗</span></button><button class="text-button full" data-action="menu">Back to menu</button>`;}
