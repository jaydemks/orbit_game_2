...
export class UI {
  constructor(callbacks = {}) {
    ...
    this.root.addEventListener('input', e => {
      if(e.target.id==='score-alias')this.syncSubmission();
  ...
  action(action, data = {}) {
    ...
    if (action === 'play') cb.onPlay?.();
      ...
      if(this.submissionIdentity==='alias'&&!/^[a-zA-Z0-9 _-]{3,24}$/.test(alias)){this.root.querySelector('#score-alias')?.focus();return;}
  ...
  renderLeaderboard() {
    ...
    const profile = username => /^[a-zA-Z0-9-]{1,39}$/.test(String(username||'')) ? `<a href="https://github.com/${encodeURIComponent(username)}" target="_blank" rel="noopener noreferrer" aria-label="Open ${esc(username)} on GitHub">@${esc(username)} ↗</a>` : `@${esc(username||'Explorer')}`;
  ...
  syncSubmission() {
    ...
    const input=this.root.querySelector('#score-alias'), valid=!aliasMode||/^[a-zA-Z0-9 _-]{3,24}$/.test(input?.value.trim()||'');
    if(input)input.setAttribute('aria-invalid',String(aliasMode&&!!input.value&&!valid));
    const submit=this.root.querySelector('[data-action="submit-confirm"]');if(submit)submit.disabled=!this.submission.available||!valid;
  }
  ...
  showDialog(type) {
    ...
    if(type==='submit')body=`<p class="dialog-description">Choose how your name appears in the rankings. A GitHub account is required for both options.</p><div class="identity-options" role="group" aria-label="Ranking identity"><button data-action="submit-identity" data-identity="github">Use GitHub username</button><button data-action="submit-identity" data-identity="alias">Make yourself famous</button></div><div id="alias-field" hidden><label for="score-alias">Your explorer alias</label><input id="score-alias" type="text" inputmode="text" autocomplete="nickname" minlength="3" maxlength="24" pattern="[A-Za-z0-9 _\\-]{3,24}" aria-describedby="alias-hint" placeholder="Cosmic explorer"><small id="alias-hint">3–24 letters, numbers, spaces, underscores or hyphens.</small></div><p class="submission-explanation">This opens a prepared GitHub issue. Sign in and confirm the submission there. Your replay must pass verification before it appears in the rankings. An alias changes your display name; your GitHub issue remains public.</p><button class="primary-button full" data-action="submit-confirm">Submit verified run <span>↗</span></button><p class="submission-status" role="status"></p><button class="text-button full" data-action="close">Back to results</button>`;
